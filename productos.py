...
def editar_producto(lista_productos):
    ...
    top_editar.geometry("600x450")

    frame_editar = ttk.LabelFrame(top_editar, text="Editar Producto")
    frame_editar.pack(padx=10, pady=10, fill="both", expand=True)

    ttk.Label(frame_editar, text="Marca Producto").pack()
    entry_marca = ttk.Entry(frame_editar)
    entry_marca.insert(0,marca)
    entry_marca.pack()

    ttk.Label(frame_editar, text="Descripcion").pack()
    entry_descripcion = ttk.Entry(frame_editar)
    entry_descripcion.insert(0,descripcion)
    entry_descripcion.pack()

    ttk.Label(frame_editar, text="Precio Venta").pack()
    entry_precio_venta = ttk.Entry(frame_editar)
    entry_precio_venta.insert(0,float(str(precio_venta).split(" $ ")[1]))
    entry_precio_venta.pack()

    ttk.Label(frame_editar, text="Precio Compra").pack()
    entry_precio_compra = ttk.Entry(frame_editar)
    entry_precio_compra.insert(0,float(str(precio_compra).split(" $ ")[1]))
    entry_precio_compra.pack()

    ttk.Label(frame_editar, text="Cantidad").pack()
    entry_stock = ttk.Entry(frame_editar)
    entry_stock.insert(0,stock)
    entry_stock.pack()

    ttk.Label(frame_editar, text="Descuento").pack()
    entry_descuento = ttk.Entry(frame_editar)
    entry_descuento.insert(0,str(descuento.split(" % ")[0]))
    entry_descuento.pack()

    ttk.Label(frame_editar, text="Proveedores").pack()
    ...
    ttk.Label(frame_editar, text="Categorias").pack()
